...
/* 
  ...
  To use Google Maps:
  1. Replace GOOGLE_MAPS_API_KEY with your valid Google Maps API Key.
...
export function MapView({ pins, onOpenDetail }: Props) {
  ...
  if (!GOOGLE_MAPS_API_KEY || GOOGLE_MAPS_API_KEY === "YOUR_API_KEY_HERE" || GOOGLE_MAPS_API_KEY.trim() === "") {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center bg-gray-50 text-gray-800 p-6 text-center">
        <AlertTriangle className="text-yellow-500 mb-3" size={32} />
        <h3 className="font-extrabold text-[15px] mb-1">Google Maps API Key Missing</h3>
        <p className="text-[12px] text-gray-500 max-w-xs mb-4">
          Please add a valid Google Maps API Key to MapView.tsx to load the safety tracker map.
        </p>
      </div>
    );
  }
